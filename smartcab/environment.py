    ...
    def step(self):
        ...
        if self.primary_agent is not None:
            if self.enforce_deadline and self.agent_states[self.primary_agent]['deadline'] <= 0:
                ...
                fo.write("{}\t{}\t{}\n".format(float(self.get_deadline(agent)) / agent.initial_deadline, agent.penalties, 0))
                fo.close
    ...
    def act(self, agent, action):
        ...
        if agent is self.primary_agent:
            if state['location'] == state['destination']:
                ...
                fo = open("output.csv", "a+")
                fo.write("{}\t{}\t{}\n".format(float(self.get_deadline(agent)) / agent.initial_deadline, agent.penalties, 1))
                fo.close
